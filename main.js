    ...
    array
      .map((item) => {
        ...
        if (item.comments && item.comments.length > 0) {
          let htmlForComments = item.comments
            .map((comment) => {
            ...
            `<div class="card nguyen-card" style="width: 100%;">
                <div class="card-body nguyen-card-body d-flex">
                    ...
                    <div class="right col-10">
                        ...
                        <a href="#" class="card-link" onclick="toggleLike('${
                          item.id
                        }')">${checkIfUserHasLike(item) ? "Unlike" : "Like"}</a>
                        <a href="#" class="card-link" onclick="comment('${
                          item.id
                        }')">Comment</a>
                        <a href="#" class="card-link" onclick="retweet('${
                          item.id
                        }')">Retweet</a>
                        <a href="#" class="card-link" onclick="deleteTweet(${
                          item.id
                        })">Delete</a>
                    </div>      
                </div>
    ...
    .map((item, index) => {
      return `<div id="newsArea" style="padding: 1em; border-top: 1px solid #D3D3D3; background-color: rgb(250,250,250)">
           ...
           <div>
               ...
               <div style="font-style: italic; font-size: 0.5em">${
                 item.author
               }</div>
               <a href="${
                 item.url
               }" style="font-size:0.5em">Get Full Coverage</a> 
                <div style="font-size: 0.7em">${item.description}</div>
                <div style="font-size: 0.7em">${moment(
                  item.publishedAt,
                  "YYYYMMDD"
                ).fromNow()}</div>
           </div>   
...
function toggleLike(tweetID) {
  ...
  else targetTweet.likes.push(currentUser); // pushing currentUser to likes array
...
function deleteTweet(deleteID) {
  ...
  tweetList = tweetList.filter(
    (tweet) => tweet.id !== deleteID && tweet.originTweetID !== deleteID
  );
  render(tweetList);
}
